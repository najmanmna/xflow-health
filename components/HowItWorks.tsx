...
const HowItWorks = () => {
  const steps = [
    {
      Icon: FileText, // Pass component reference, not JSX
  ...
  return (
    <section id="how-it-works" className="py-12 md:py-16 bg-primary relative overflow-hidden">
      {/* Background Decor - Adjusted for smaller height */}
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8 relative">
          ...
          {/* Connector Line (Desktop Only) - Repositioned */}
          <div className="hidden md:block absolute top-12 left-[20%] right-[20%] h-[2px] bg-gradient-to-r from-transparent via-white/10 to-transparent -z-10" />

          {steps.map((step, index) => (
            <div 
              key={index} 
              className="group relative flex flex-col items-center text-center p-6 rounded-2xl bg-secondary/30 border border-white/5 shadow-sm hover:shadow-xl hover:shadow-accent/5 transition-all duration-500 hover:-translate-y-1 backdrop-blur-sm"
            >
              {/* Step Number (Big Background) */}
              <span className="absolute top-2 right-4 text-7xl font-bold text-white/5 -z-10 select-none group-hover:text-accent/5 transition-colors duration-500">
                {index + 1}
              </span>

              {/* Icon Circle - Now controls Icon color via group-hover */}
              <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-accent group-hover:rotate-6 transition-all duration-500 relative overflow-hidden border border-white/10">
                ...
                <div className="relative z-10">
                  {/* Icon with direct color transition classes */}
                  <step.Icon className="w-8 h-8 text-accent transition-colors duration-300 group-hover:text-primary" />
                </div>
              </div>
